...
const string world_file = "../resources/11-UnifiedHapticControlForceSpaceIdentification/world.urdf";
const string robot_file = "../resources/11-UnifiedHapticControlForceSpaceIdentification/panda_arm_sim.urdf";
...
int main() {
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (fMouseYp) {
			current_mouse_position[1] += 0.0001;
		}
		if (fMouseYn) {
			current_mouse_position[1] -= 0.0001;
		}
		if (fMouseZp) {
			current_mouse_position[2] += 0.0001;
		}
		if (fMouseZn) {
			current_mouse_position[2] -= 0.0001;
		}
		if (fMouseGripper || k>=1)
		{
			mouse_gripper = 1;
			k ++;
		}
...
void simulation(Sai2Model::Sai2Model* robot, Simulation::Sai2Simulation* sim) {
	...
	while (fSimulationRunning) {
		...
		// read force controlled direction
		contact_control_first = redis_client.getEigenMatrixJSON(CONTROLLED_CONTACT_DIRECTION_1);
		contact_control_second = redis_client.getEigenMatrixJSON(CONTROLLED_CONTACT_DIRECTION_2);
		contact_control_third = redis_client.getEigenMatrixJSON(CONTROLLED_CONTACT_DIRECTION_3);
